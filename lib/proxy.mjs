 ...
 * transport 自己持有本机宿主」），全仓库仅此一处读取——无其他副作用。
 *
 * 本补丁把该旗标置 true：经本代理的访问确实满足这个语义——代理与宿主同机，把入站
 * 请求的 Host / Origin 回环化后转发给 127.0.0.1:3080，且带 PIN 鉴权。因此它比包装
 * 模块加载器更贴合上游契约，作为主修法；下面的 LOOPBACK_ENV_PATCH 保留作双保险。
 *
 * 与上游 TRANSPORT_API_CLIENT_SHIM 共存的关键：那个 shim 也把 __DSH_TRANSPORT__
 * 装成了访问器。本脚本必须**注入在它之后**，并链式委托原有的 get/set——先让它补
 * createApiClient，再补 ownsHost，两个补丁叠加而不互相覆盖。
 */
export const TRANSPORT_OWNS_HOST_PATCH = `<script data-dsh-pocket-owns-host-patch="1">!function(){try{
var K='__DSH_TRANSPORT__';
function force(t){if(t&&(typeof t==='object'||typeof t==='function')){try{Object.defineProperty(t,'ownsHost',{value:true,writable:true,configurable:true});}catch(e){try{t.ownsHost=true;}catch(e2){}}}return t;}
var d=Object.getOwnPropertyDescriptor(globalThis,K);
var own=d&&d.get?undefined:(d?d.value:undefined);
function read(){return (d&&d.get)?d.get.call(globalThis):own;}
function write(v){own=v;if(d&&d.set){d.set.call(globalThis,v);}}
try{
  Object.defineProperty(globalThis,K,{configurable:true,enumerable:true,
    get:function(){return force(read());},
    set:function(v){write(v);force(read());}});
}catch(e){var c0=read();if(c0)force(c0);}
var early=read();if(early)force(early);
}catch(e){}}();</script>`;
