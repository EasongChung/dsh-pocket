 ...
 *
 * 上游曾尝试「全局 let location + Proxy」伪装 hostname，因词法绑定冲突导致会话列表
 * 回归而回退。本补丁**不碰 location**：包装 window.__ModuleLoader__，在
 * dsh-client-connection 模块向 Cordis 容器 provide('connection') 时，把服务句柄的
 * isLoopback 强制为 true——经由本代理（带 PIN 鉴权、请求头已回环化）的远程访问
 * 由此获得与本机一致的完整设置能力。宿主侧特权 RPC 栅栏由 loopbackAuthority 的
 * Host/Origin/Referer/Sec-Fetch-Site 规范化负责放行。
 *
 ...
 * 栅栏（settings.describe/credentials.* 等 PRIVILEGED_METHODS）会校验
 * sec-fetch-site === 'cross-site' 拒绝与 Origin===Host 匹配，远程访问的这两个头
 * 若不改写，设置/凭据平面会在宿主侧被 403（与 issue #58 同源的另一半问题）。
