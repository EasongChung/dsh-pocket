...
/**
 * issue #58（DSH 0.1.5-rc.2 起的主修法）：宿主连接层把 isLoopback 改为
 *   isLoopback: transport?.ownsHost === true || pageLocation === void 0
 *               || isLoopbackHostname(pageLocation.hostname)
 * 其中 transport = globalThis.__DSH_TRANSPORT__（apply() 时即时读取，见该 bundle
 * 第 6307 / 6344 行）。`ownsHost` 是宿主留给**桌面外壳**的官方旗标（含义即「本
 * transport 自己持有本机宿主」），0.1.5-rc.2 全仓库仅此一处读取——无其他副作用。
 *
 * 关键事实（真机复盘）：浏览器端**从来没有人给 __DSH_TRANSPORT__ 赋值**——web
 * frontend 只读它一次（`const i=globalThis.__DSH_TRANSPORT__`，仅用于可选的
 * `i?.loadBundle`），赋值只发生在桌面外壳里。所以「已有对象时改旗标」的写法在
 * 手机上等于什么都没做（`transport?.ownsHost` 仍是 undefined !== true）。本补丁
 * 因此在读取时**物化**一个 transport 对象并置 ownsHost=true：经本代理的访问确实
 * 满足该语义——代理与宿主同机，把入站 Host / Origin 回环化后转发给
 * 127.0.0.1:3080，且带 PIN 鉴权。
 *
 * 物化时必须一并补 createApiClient（返回 null，让宿主走它自己的
 * `?? new WebApiClient()` 兜底）：连接层写的是 `transport?.createApiClient()`，
 * 可选链只挡 nullish、不挡方法缺失，少了它就是上游 issue #96 的整页 TypeError。
 * 桌面外壳自带实现时一律不覆盖。
 *
 * 与上游 TRANSPORT_API_CLIENT_SHIM 共存：那个 shim 也把 __DSH_TRANSPORT__ 装成
 * 访问器，故本脚本**注入在它之后**并链式委托原有 get/set（先让它补，再补 ownsHost），
 * 两个补丁叠加而不互相覆盖。下面的 LOOPBACK_ENV_PATCH 保留作双保险。
 *
 * 自检标记 window.__DSH_POCKET_LOOPBACK__ = {ownsHost, created}：created 为 true
 * 表示浏览器本来没有 transport、由本补丁物化——排查时直接在控制台读它。
 */
export const TRANSPORT_OWNS_HOST_PATCH = `<script data-dsh-pocket-owns-host-patch="1">!function(){try{
var K='__DSH_TRANSPORT__';
var made=false;
function force(t){
  if(t&&(typeof t==='object'||typeof t==='function')){
    try{Object.defineProperty(t,'ownsHost',{value:true,writable:true,configurable:true});}catch(e){try{t.ownsHost=true;}catch(e2){}}
    try{if(typeof t.createApiClient!=='function'){Object.defineProperty(t,'createApiClient',{value:function(){return null;},writable:true,configurable:true});}}catch(e){try{if(!t.createApiClient){t.createApiClient=function(){return null;};}}catch(e3){}}
  }
  return t;
}
var d=Object.getOwnPropertyDescriptor(globalThis,K);
var own=d&&d.get?undefined:(d?d.value:undefined);
function raw(){return (d&&d.get)?d.get.call(globalThis):own;}
function store(v){own=v;if(d&&d.set){try{d.set.call(globalThis,v);}catch(e){}}}
function ensure(){var v=raw();if(v===null||v===undefined){v={};made=true;store(v);v=raw()||v;}return force(v);}
try{Object.defineProperty(globalThis,K,{configurable:true,enumerable:true,get:ensure,set:function(v){store(v);force(raw()||v);}});}catch(e){try{ensure();}catch(e4){}}
var t0=ensure();
try{window.__DSH_POCKET_LOOPBACK__={ownsHost:!!(t0&&t0.ownsHost===true),created:made};}catch(e){}
}catch(e){}}();</script>`;

/**
 * issue #58（0.1.1-rc.2 时代的方案，保留作双保险）：DSH 客户端用 location.hostname 判定 connection.isLoopback；手机/局域网
 * 访问时非回环 → dsh-client-ui-settings 把 settings 镜像降级为 memory 模式，模型/通用
 * 设置页报 "settings are unavailable in this browser"。
 *
 * 与已回退的「全局 let location + Proxy」伪装方案不同，本补丁**不碰 location**：
 * 包装 window.__ModuleLoader__，在 dsh-client-connection 模块向 Cordis 容器
 * provide('connection') 时把服务句柄的 isLoopback 强制为 true。经本代理（带 PIN
 * 鉴权、请求头已回环化）的远程访问由此获得与本机一致的完整设置能力，会话列表回归
 * 不存在（issue #58 的可行修法，DSH 0.1.5-rc.2 验证仍适配）。
 *
 ...
 * + issue #58 的 loopback 信任环境补丁。
 */
export const DEFAULT_INJECT = RANDOM_UUID_POLYFILL + TRANSPORT_API_CLIENT_SHIM + TRANSPORT_OWNS_HOST_PATCH + LOOPBACK_ENV_PATCH;
