...
export async function startNamedTunnel({ token, home, signal, onPhase = () => {} }) {
  ...
  await new Promise((resolve, reject) => {
    ...
    const onExit = (code) => {
      ...
      reject(new Error(
        `cloudflared 退出（code=${code}）${tail ? '：' + tail.slice(0, 500) : ''}——请检查 Tunnel Token 是否有效、域名 Service 是否指向本机代理端口 | `
        + `tunnel exited (code=${code})${tail ? ': ' + tail.slice(0, 500) : ''} — check the Tunnel Token and the ingress hostname`,
      ));
    };
    cleanup = () => {
